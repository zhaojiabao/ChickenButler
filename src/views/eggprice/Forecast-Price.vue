<template>
	...
	<div class="price-forecast">
		...
		<div class="rule-text">
			...
			<router-link to="/p/forecast-active"> <span @click="lookActive">查看活动进程</span></router-link>

		</div>

		<router-link to="/p/forecast-record" class="forecast-btn theme-bg">
			<span @click="ceJilu">预测记录</span>
		</router-link>
	</div>
</template>
...
<script>
	...
	import SaleAddress from '../newegg/NewSaleAddress'	
	...
	export default {
		...
		mounted() {
			...
			if(this.$store.state.ticket == null) {
				this.$vux.toast.text('请登录后查看蛋价预测!')
				this.$router.push('/p/login')
			}
			if(this.$store.state.egg_price.area.indexOf(',') > -1) {
				this.init();
			}
			//this.init();
			userAction({
		...
		methods: {
			init() {
				...
				this.$wechat.ready(() => {
					var title = '快来参与主产区' + this.$store.state.egg_price.area + '蛋价预测吧-预测得好礼';
					var link = 'https://m.danjiguanjia.com/#/p/price-forecast';
