				...
				<div class="forum-detail-author-right">
					...
					<div class="ask-egg">
						...
						<div class="ask-egg-right" v-if="forums.type==2">
							<span class="egg">提问积蛋: {{forums.prize_scores}}枚</span><span class="ask" v-if="forums.is_adopt==0">提问中</span><span class="finish" v-else>已解决</span>
						</div>
					</div>
				</div>
			</div>
				...
				this.$vux.confirm.show({
					...
					onConfirm() {
						delCommunity({
							...
							if(res.retcode == "2000") {
								_this.$vux.toast.text(res.msg);
								_this.$router.push('/p/forum-list/4')
							} else {
								_this.$vux.toast.text('');
							}
						})
