<template>
	...
	<div class="forum">
			...
			<div class="forum-list" v-for="(forum,index) in forum_list" @click="forumClick(forum.id)">
				<div class="forum-main">
					...
					<div class="forum-main-right">
						...
						<div class="from-tag">
							<p v-if="forum.circle!=''" class="from-tag-from">来源: {{forum.circle}}</p>
							<p v-for="(tag,index) in forum.tag_id" v-if="index<2" class="tag from-tag-tag"><span class="state">{{tag}}</span></p>
							<p v-else class="from-tag-from"> </p>
						</div>
					</div>
				</div>
			</div>
		...
		<div v-else>
			...
			<div class="circle-main" v-for="(circle,index) in circle_list" v-show="circle.is_circle==0" @click="circleClick(circle.id)">
				<div class="circle-main-left">
				...
				<div class="circle-main-right">
					...
					<span class="join" @click.stop @click="joinCircle(circle)">加入</span>
				</div>
			</div>
		</div>
...
<script>
	...
	import { forumsList, circleList, circleJoin, userInfo } from "../../api/api"
	...
	export default {
		...
		mounted() {
			// 从帖子返回选择数据
			if(this.$store.state.forum_selected_index > -1) {
				this.$route.params.index = this.$store.state.forum_selected_index;
			}
		...
		methods: {
			...
			onItemClick(index) {
				...
				this.$store.state.forum_selected_index = index;
				if(index < 3) {
					this.$store.state.header.right_title = "我的论坛";
					this.$store.state.header.right_url = "/p/myforum";
				} else {
			...
			btnClick() {
				...
				if(audios1.paused) {
					audios1.play();
					audios1.src = this.forum.content;
				} else {
					audios1.pause();
				}
			}
