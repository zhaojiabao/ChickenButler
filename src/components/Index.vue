	...
	<div class="home">
		<div class="index">
			...
			<div class="home-function">
				<swiper :list="banner_list" dots-position="center" :show-desc-mask="false" :auto="true" :loop="true" :interval="1500" @click="banner_click"></swiper>
				<grid :rows="5" class="grid">
					<grid-item :link="module.path" v-for="(module,index) in module_list" @on-item-click="onItemClick(index)">
				...
				<div class="news-list" v-for="(news,index) in news_list">
					<div class="new-list-main" @click="newsClick(news.cms_id)">
						<template v-if="news.video_url!=''">
							...
							<video width="100%" :src="news.video_url" webkit-playsinline="true" x5-video-player-type="h5" :poster="news.cms_pic" controls="controls"></video>
							<ul>
			...
			return {
				...
				module_list: [{
					...
					},
					{
						...
						title: '供求服务',
						img: supply,
						path: 'https://m.danjiguanjia.com/supplydemand/#/?ticket='+localStorage.getItem('ticket')+'&h5url=supplydemand'
					}
				],
				page: 0
			}
...
//					ticket:localStorage.getItem('ticket')
			...
			onItemClick(index_grid) {
				...
				} else if(index_grid == 1) {
					userAction({
						ticket: this.$store.state.ticket,
						module: 15,
						cur_version: this.$store.state.cur_version
					})
				} else if(index_grid == 2) {
					userAction({
						ticket: this.$store.state.ticket,
						module: 16,
						cur_version: this.$store.state.cur_version
